...
public:
    ...
    explicit Router(const Graph& graph, RoutesInternalData);

    const std::vector<std::vector<std::optional<RouteInternalData>>>& GetRoutesData() const;
...
template <typename Weight>
const std::vector<std::vector<std::optional<typename Router<Weight>::RouteInternalData>>>& Router<Weight>::GetRoutesData() const {
